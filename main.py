...
class RAGSystem:
    ...
    async def setup_retrievers(self, persist_directory: str = "./chroma_db"):
        ...
        try:
            logger.info(f"🔍 Setting up vector store in {persist_directory}...")
...
async def hackrx_run(request: HackRxRequest):
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:  # Increased timeout for GCP
        ...
        chroma_db_path = os.path.join(temp_dir, "bajaj_chroma_db")
        ...
        # 4. Process each question with BAJAJ Insurance analysis
        for i, question in enumerate(request.questions):
            logger.info(f"[{session_id}] BAJAJ Insurance: Processing question {i+1}/{len(request.questions)}: '{question}'")
            ...
            # Use the comprehensive final answer for the response
            answers.append(decision_result.get("final_answer", "No specific answer could be formulated using BAJAJ Insurance analysis."))
