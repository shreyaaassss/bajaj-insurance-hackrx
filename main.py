...
# Memory optimization for RAM constraints
OPTIMAL_BATCH_SIZE = 16  # Good balance for 8GB RAM machines
...
async def get_embeddings_batch(texts: List[str]) -> List[np.ndarray]:
    """Process embeddings with smart caching and memory optimization - FIXED"""
    if not texts:
        return []
    ...
    # Process uncached texts with memory optimization
    if uncached_texts:
        try:
            await ensure_models_ready()
            if base_sentence_model:
                # MEMORY OPTIMIZATION: Process in batches to avoid memory spikes
                ...
                # Cache embeddings using cache manager
